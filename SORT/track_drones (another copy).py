...
# 		plt.show()
	...
	def getData(self, trackers):
		...
		for d in trackers:
			...
			if self.use_gt:
				an_data["dco"] = True
			an_data["height"] = round((d[3] - d[1]),2)
			an_data["width"] = round((d[2] - d[0]),2)
			an_data["id"] = (d[4])
			an_data["x"] = round((d[0]),2)
			an_data["y"] = round((d[1]),2)
...
# 			ax1.imshow(img)
			if (self.display):
				...
				fig.savefig('output/'+str(f_id).zfill(4)+'_final_seg.png', dpi=50)                
				#save the frame with tracking boxes
				ax1.cla()
		
		if self.use_gt:
			filename = os.path.join(self.output_dir,"gt.json")
		else:
